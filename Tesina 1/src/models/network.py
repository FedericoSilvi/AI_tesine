...
class WirelessNetwork:
    ...
    def get_node_colors(self, scenario: str = 'smartcity') -> List[str]:
        """
        Return colors for nodes based on their properties and scenario.
        
        Args:
            scenario: Type of scenario ('smartcity', 'seismic', or 'energy')
        """
        colors = []
        for node_id in sorted(self.nodes.keys()):
            node = self.nodes[node_id]
            
            if scenario == 'smartcity':
                # Color based on priority (using terrain_difficulty as proxy)
                # Higher terrain_difficulty = higher priority sensors
                if node.terrain_difficulty > 1.5:
                    colors.append('red')  # High priority (Traffic sensors)
                elif node.terrain_difficulty > 1.2:
                    colors.append('orange')  # Medium priority (Environmental)
                else:
                    colors.append('lightblue')  # Low priority (Smart lights)
    ...
    def validate_mst_constraints(self, 
        ...
        if scenario == 'smartcity':
            ...
            for n1, n2 in mst_edges:
                node1 = self.nodes[n1]
                node2 = self.nodes[n2]
                # Latency is approximately 0.1ms per unit distance
                estimated_latency = node1.distance_to(node2) * 0.1
                if estimated_latency > constraints.get('max_latency', float('inf')):
                    return False
